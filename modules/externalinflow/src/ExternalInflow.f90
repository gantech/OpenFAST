...
!..................................................................................................................................
   ...
   ELSEIF ( p_FAST%CompAero  == Module_AD ) THEN ! AeroDyn 15 needs these velocities
      ...
      ExtInfw%p%NnodesVel = ExtInfw%p%NnodesVel + y_AD%rotors(1)%TowerLoad%NNodes                   ! tower nodes (if any)
      DO k=1,ExtInfw%p%NumBl
         ExtInfw%p%NnodesVel = ExtInfw%p%NnodesVel + u_AD%rotors(1)%BladeMotion(k)%NNodes           ! blade nodes
      END DO
   END IF
   ...
   DO k=1,ExtInfw%p%NumBl
      ...
      ELSEIF (p_FAST%CompElast == Module_BD ) THEN
         ...
         call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
      END IF
      call MeshMapCreate( y_AD%rotors(1)%BladeLoad(k), ExtInfw%m%ActForceLoads(k), ExtInfw%m%Line2_to_Line2_Loads(k),  ErrStat2, ErrMsg2 );
      call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
...
!      ExtInfw%m%ActForceLoads(k)%RemapFlag = .false.
   ...
   do k=ExtInfw%p%NumBl+1,ExtInfw%p%NMappings
      ...
      if ( y_AD%rotors(1)%TowerLoad%nnodes > 0 ) then ! we can have an input mesh on the tower without having an output mesh.
         call MeshMapCreate( y_AD%rotors(1)%TowerLoad, ExtInfw%m%ActForceLoads(k), ExtInfw%m%Line2_to_Line2_Loads(k),  ErrStat2, ErrMsg2 );
         call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
         call MeshMapCreate( ExtInfw%m%ActForceLoads(k), ExtInfw%m%ActForceLoadsPoints(k), ExtInfw%m%Line2_to_Point_Loads(k),  ErrStat2, ErrMsg2 );
         call SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
!         ExtInfw%m%ActForceLoads(k)%RemapFlag = .false.
  ...
  TYPE(ExternalInflow_Data),            INTENT(INOUT)   :: ExtInfw        ! data for the ExternalInflow integration module
   ...
   DO K = 1,SIZE(u_AD%rotors(1)%BladeMotion)
      DO J = 1,u_AD%rotors(1)%BladeMotion(k)%Nnodes
         ...
         ExtInfw%u%pxDotVel(Node) = u_AD%rotors(1)%BladeMotion(k)%TranslationVel(1,j)
         ExtInfw%u%pyDotVel(Node) = u_AD%rotors(1)%BladeMotion(k)%TranslationVel(2,j)
         ExtInfw%u%pzDotVel(Node) = u_AD%rotors(1)%BladeMotion(k)%TranslationVel(3,j)
   ...
   if (ExtInfw%p%NMappings .gt. ExtInfw%p%NumBl) then
      ...
      DO J=1,u_AD%rotors(1)%TowerMotion%nnodes
         ...
         ExtInfw%u%pxDotVel(Node) = u_AD%rotors(1)%TowerMotion%TranslationVel(1,J)
         ExtInfw%u%pyDotVel(Node) = u_AD%rotors(1)%TowerMotion%TranslationVel(2,J)
         ExtInfw%u%pzDotVel(Node) = u_AD%rotors(1)%TowerMotion%TranslationVel(3,J)
      END DO
   end if
...
SUBROUTINE ExtInfw_InterpolateForceNodesChord(InitOut_AD, p_ExtInfw, u_ExtInfw, ErrStat, ErrMsg)
  ...
  do k = 1, p_ExtInfw%NumBl
     ...
     AD_etaNodes = InitOut_AD%rotors(1)%BladeProps(k)%BlSpn(:)/InitOut_AD%rotors(1)%BladeProps(k)%BlSpn(nNodesBladeProps)
  ...
  do k = p_ExtInfw%NumBl+1,p_ExtInfw%NMappings
     ...
     ! Calculate the chord at the force nodes based on interpolation
     AD_etaNodes = InitOut_AD%rotors(1)%TwrElev(:)/InitOut_AD%rotors(1)%TwrElev(nNodesTowerProps) ! Non-dimensionalize the tower elevation array
